                ...
                new html.DefaultHandler(
                    function(error, dom) {
                        ...
                        if(error) {
                            ...
                            DJSUtil.log('PARSER ERROR: ' + error);
                        } else {
 ...
 */
        dominate: function() {
            ...
            document.write = document.writeln = function(out) {
                ...
                DJSUtil.log('Caught document.write: ' + out);
                self.write(out);
            }
            ...
            if(document.addEventListener) {
                ...
                document.attachEvent = function(event, handler) {
                    ...
                    if(handlers[trimmedEvent]) {
                        ...
                        nativeMethods.attachEvent.apply(document, arguments);
                    }
                };
                ...
                DJSUtil.forEach(
                    ...
                    function(handlerArray, eventName) {
                        ...
                        nativeMethods.attachEvent.call(
 ...
 */
        write: function(out) {
            ...
            if(parser) {
                ...
                DJSUtil.log('Warning: document.write was called, but no parser was present!');
 ...
 */
        flush: function() {
            ...
            if(self.hasWriteBuffer) {
                ...
                DJSUtil.log('Flushing the buffer...');
 ...
 */
        createElement: function(abstractElement) {
            ...
            DJSUtil.log('Creating element with type ' + abstractElement.type);
 ...
 */
        insert: function(abstractDOM, parent) {
            ...
            DJSUtil.log('Got abstract DOM object for insertion!');
            DJSUtil.inspect(abstractDOM);
            
            DJSUtil.forEach(
                abstractDOM,
                function(data) {

                    var node = self.createElement(data);
 ...
 */
        whenLoaded: function(callback) {
            ...
            (nativeMethods.addEventListener || nativeMethods.attachEvent).call(window, 'load', callback, true);
        },
/*
 * DJSWindow.load
 *
        ...
        execute: function(callback) {
            ...
            var self = this,
                ...
                flushBuffer = function() {
                    ...
                    // TODO: See if we can find a more automated way
                    // to flush the buffer. Methinks event system is
                    // called for..
                    slaveDocument.flush();
                };
            ...
            if(self.external) {
                ...
                try {
                    ...
                    // TODO: If we fail inline execution, somehow reset the 
                    // slaveDocument buffer...
                    callback(false);
                }
 ...
 */
    ...
    DJSScriptManager.prototype = {
        ...
        dominate: function() {
            ...
            DJSUtil.forEach(
                ...
                function(script, index) {
                    ...
                    if(!script.dominated && DJSUtil.getAttribute.call(script, 'type') === 'text/djs') {
                        ...
                        if(!(urlCache[slave.src] || self.executing)) {
                            slave.precache(
            ...
            DJSUtil.log('Enslaved ' + enslavedCount + ' scripts...');
        },
        execute: function(callback) {
            ...
            DJSUtil.log('Execute has been called!');

            DJSUtil.log('Captives: ' + captives.length);
            if(captives.length) {

                var slave = captives[0];

                if(slave.ready && !self.currentExecution) {

                    DJSUtil.log('Attempting to execute next script...');
                ...
                DJSUtil.log('Fallback..');
                self.dominate();
            }

            if(!captives.length && self.executionCallback) {

                DJSUtil.log('Looks like we are done dominating scripts!');
                self.executionCallback();
                self.executionCallback = null;
    ...
    DJSUtil.log('Initializing DominateJS... ');
    ...
    slaveWindow.whenLoaded(
        ...
        function() {
            ...
            DJSUtil.log('Got the load event. Executing scripts!');

            slaveScripts.execute(

                function() {

                    DJSUtil.log('Script execution complete. Simulating events!');

                    slaveDocument.ready();
                    slaveWindow.load();

                    if(options.restoreAfterExecution) {

                        DJSUtil.log('Got the safe word. Restoring native DOM methods!');
