...
*******************************************************************************/
/*******************************************************************************
This is an experimental, modified fork of ControlJS.
  - fork code project: https://github.com/cdata/cloudflare-controljs
*******************************************************************************/
var CJS = window.CJS || {},
	htmlparser = typeof exports == "object" ? exports : false;
...
CJS.start = function() {
	...
	if ( CJS.defer ) {
		...
		// Sorry, don't want to be this noisy about it :P
		//alert("Immediate processing is not currently supported.");
		CJS.dprint("Immediate processing is not currently supported.");
		CJS.processScripts();
	}
};
...
CJS.downloadScriptImage = function(url) {
	var img = new Image();
	img.onload = function() { CJS.onloadCallback(url, img); };
	img.onerror = function() { CJS.onloadCallback(url, img); }; // Chrome does onerror (not onload).
	img.src = url;
};
...
if ( "undefined" != typeof(console) && "undefined" != typeof(console.log) && CJS.verbose) {
...
if(htmlparser) {
	...
	CJS.docwrite = (
		function(){
			...
			var scriptStack = [CJS.curScript],
				currentScript = function() {
					
					return scriptStack.length ? scriptStack[0] : false;
				},
				addScript = function(script) {
					
					scriptStack.unshift(script);
				},
				next = function() {
			...
			var handler = new htmlparser.DefaultHandler(
					function(error, dom) {
						...
						if(error) {
							...
							var parentNode = CJS.curScript.parentNode,
								cursor = CJS.curScript,
								...
								createNode = function(nodeData) {
									...
									switch(nodeData.type) {
										...
										case 'comment':
										case 'text':
											console.log('Creating text node with value: ' + nodeData.data);
											return document.createTextNode(nodeData.data);
											break;
										case 'script':
										case 'tag':
											
											console.log('Creating element: ' + nodeData.raw);
											...
											break;
										default: 
											console.log('WARNING: I have no idea what this is: ' + nodeData.raw);
											console.info(nodeData);
											break;
									}
								},
								walkDOM = function(nodeData, domParent) {
									
									console.log("Walking across " + nodeData.length + " elements:");
									console.info(nodeData);
									
									for(var i = 0; i < nodeData.length; i++) {
										
										console.log("Hitting child " + i);
										
										(function() {
											
											var childData = nodeData[i],
												node = createNode(childData);
											
											if(childData.children) {
												
												console.log("Element has " + childData.children.length + " children; diving in...");
												
												setTimeout(
													function() {
														
														walkDOM(childData.children, node);
													},
													10
												);
											}
											...
											if(domParent) {
												...
												console.log("Appending element to " + domParent.nodeName);
												console.info(node);
												...
												try {
													...
													console.log("Nope, that doesn't work! " + e);
													console.log("Parent: ");
													console.info(domParent);
													console.log("Node: ");
													console.info(node);
												}
											} else {
												...
												if(parentNode.nodeName.toLowerCase() == 'head') {
													...
													console.log("Inserting element before " + cursor.nodeName + " in " + parentNode.nodeName);
													
													parentNode.insertBefore(node, cursor);
												}
											}
				...
				console.log("Attempting to write the following fragment: " + arguments[0]);
				parser.parseComplete(arguments[0]);
			
		} else {
			
			CJS.dprint("Something went wrong.. ");
		}
	};
